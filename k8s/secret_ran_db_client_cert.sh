...
CLIENT_KEY=$(op document get "ran_db_client_prod_key" --vault world_site | base64 -w 0)

# check if values are retrieved successfully
if [[ -z "$CLIENT_CERT" || -z "$CLIENT_KEY" ]]; then
  echo "Error: failed to get ran prod db client certificate or key from 1Password."
  exit 1
fi
