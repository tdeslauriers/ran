...
# variables
NAMESPACE="world"
SECRET_NAME="secret-ran-server-cert"

# get certificate and key from 1Password
SERVER_CERT=$(op document get "ran_service_server_prod_cert" --vault world_site | base64 -w 0)
SERVER_KEY=$(op document get "ran_service_server_prod_key" --vault world_site | base64 -w 0)

# check if values are retrieved successfully
if [[ -z "$SERVER_CERT" || -z "$SERVER_KEY" ]]; then
  echo "Error: failed to get ran  prod server certificate or key from 1Password."
  exit 1
fi
